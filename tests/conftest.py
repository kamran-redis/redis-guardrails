...
def _redis_search_available(redis_url: str) -> bool:
    try:
...
def redis_url() -> str:
    ...
    if not _redis_search_available(url):
        pytest.skip(
            f"no Redis with search/vector support reachable at {url!r}; "
            "set REDIS_URL or run `docker run -p 6379:6379 redis:8`"
        )
    return url
...
def allow_test_overwrite() -> None:
    ...
    `redis_url` above only confirms Redis search/vector support is
    *reachable* -- that says nothing about whether it's safe to
    destructively overwrite. Reachable at redis://localhost:6379 (the
    default) could just as easily mean a real staging/dev Redis someone
    has port-forwarded to their machine for an unrelated reason, not the
    disposable `redis-guardrails` container this suite expects.
    ...
    if os.environ.get(_ALLOW_OVERWRITE_ENV_VAR) != "1":
        pytest.skip(
            ...
            "redis-guardrails container, not a real staging/dev Redis)."
        )
