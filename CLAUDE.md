...
2. **CLI** (`src/redis_guardrails/cli/`) — `core.py` is a framework-agnostic layer (zero click, zero print, returns plain dataclasses) wrapping the core library; `formatting.py` is pure string presenters (zero click); `commands.py` is thin click glue. Entry point: `redis-guardrails`. `core.py`'s `summarize_outcomes()` is the single source of benchmark pass/fail tallies + accuracy-by-category — both `format_benchmark_report` and the web Benchmarks route call it; don't recompute those numbers elsewhere.
3. **Web GUI** (`src/redis_guardrails/web/`) — FastAPI + server-rendered Jinja2 templates, launched via `redis-guardrails serve`. Routes call `cli.core` functions and `GuardrailService` directly — **never reimplement evaluation/benchmark/CRUD logic in the web layer**. `_build_app()` (routers/static/exception-handlers, no service) is split from `create_app()` (adds the service via FastAPI lifespan) specifically so tests can exercise the whole app with a `FakeStore`, no real Redis or embedding model needed. This extends to client-side JS: the Benchmarks pre-run preview table (built in JS before a real run exists) shows placeholder Actual/Result values rather than reimplementing `classify()` in the browser.
...
- `/benchmarks/run` still accepts `source=preset` + `preset_path` and validates the filename against a **live glob** of `data/*.json` before ever calling `open()` (path-traversal guard) — the web GUI's own form is upload-only, but that validation stays in the route in case anything posts to it directly.
...
- Toggling a `<tr>`'s visibility (results/preview tables' expand-row and filter logic) uses the `.hidden` DOM property only — mixing it with inline `style.display` causes stale visibility, since the inline style wins over the `[hidden]` UA rule.
...
- `redis-guardrails serve` doesn't hot-reload: Jinja templates are read fresh every request, but changes to routes/`cli/core.py` need the process restarted — kill any stale process on the port first (`lsof -i :8000`), or Python-level changes silently won't apply.
...
- Sample/test data lives in `data/` (`guardrails.json`, `testdata.json`, `testdata_extra.json`) — the CLI's `benchmark` command reads from this directory via a live glob, so new files dropped in `data/*.json` become available automatically there. The web GUI's Benchmarks page and "Run a Prompt" example browser are both upload-only (an `<input type="file">`, parsed either server-side after upload or client-side with `FileReader` for the example browser) — neither globs `data/` for display, though `/benchmarks/run`'s `source=preset` path still does for its own validation. Test-data cases use `text` (what's being checked) — not scope-specific `input`/`output` keys. There is no separate context/prior-turn field; a case that needs prior-turn context inlines it directly in `text`.
- Widget state that must survive a Run/Check form POST (full page reload) or a tab switch — Run Prompt's Settings/Examples, Benchmarks' Settings/loaded file — lives in `sessionStorage` and is restored on every page load; there's no server-side session. To let "Run" work again without re-picking a file, reconstruct it with `new File([...], name)` + `DataTransfer` assigned to the file input's `.files`.
- Space Mono is missing some Unicode glyphs (e.g. U+2212 minus sign) — use ASCII `-` for JS-generated button/glyph text in mono-font UI, not the Unicode minus.
