...
- Sample/test data lives in `data/` (`guardrails.json`, `testdata.json`, `testdata_extra.json`) — the CLI's `benchmark` command and the web GUI's benchmark preset picker read from this directory via a live glob, so new files dropped in `data/*.json` become available automatically. The web GUI's "Run a Prompt" example browser does **not** glob this directory — it's upload-only (a `<input type="file">` parsed client-side with `FileReader`, no server round trip), since it's just populating a browsable list rather than running anything server-side. Test-data cases use `text` (what's being checked) — not scope-specific `input`/`output` keys. There is no separate context/prior-turn field; a case that needs prior-turn context inlines it directly in `text`.
