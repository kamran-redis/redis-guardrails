...
- Guardrail scopes (`Scope`, in `models.py`) are open strings, not a fixed `input`/`output` enum — a new scope's `SemanticRouter` is created lazily the first time a guardrail uses it (`GuardrailStore._ensure_router`), and known scopes are tracked in a Redis `SET` (`guardrails:scopes`) so a second process can discover one created elsewhere. **A new scope name must not be a prefix of, or prefixed by, any existing scope name** — the Redis index key prefix is derived as `f"guardrails-{scope}"`, so e.g. `input` and `input2` would overlap and corrupt each other's data; `_ensure_router` rejects this.
